...
describe('jpg-to-webp.js — window.convertFile', () => {
  it('draws the source image and exports WebP with no background fill', async () => {
    const dom = createDom();
    mockImageLoad(dom.window, { width: 200, height: 150 });
    const { ctx, toBlobCalls } = mockCanvas(dom.window);
    evalScript(dom, 'converters/jpg-to-webp.js');
  ...
  it('rejects with a WebP-support hint when toBlob returns no blob', async () => {
    ...
    // Simulate a browser that can't encode WebP: toBlob calls back with null.
