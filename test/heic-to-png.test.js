...
describe('heic-to-png.js — window.convertFile', () => {
  it('calls heic2any with the file and PNG output settings, returning its result', async () => {
    const dom = createDom();
    const outputBlob = new dom.window.Blob(['fake-png'], { type: 'image/png' });
  ...
  it('takes the first blob when heic2any returns an array (multi-image HEIC)', async () => {
    const dom = createDom();
    const first = new dom.window.Blob(['page-1'], { type: 'image/png' });
    const second = new dom.window.Blob(['page-2'], { type: 'image/png' });
