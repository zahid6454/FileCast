(function () {
  ...
  window.convertText = function (text) {
    ...
    return { text: formatMarkup(text), filename: 'formatted.xml' };
  };
  ...
  // spec), and this converter always runs inside text-diff/text-converter's
  // dedicated Worker, never on the main thread. Checks: every tag closes,
  // tags nest without overlapping, exactly one root element, and bare "&"
  // outside tags is rejected (must be escaped as "&amp;").
  ...
  // Reformats the ORIGINAL text (not a DOM re-serialization), so attribute
  // order, quoting style, and self-closing style are preserved exactly —
