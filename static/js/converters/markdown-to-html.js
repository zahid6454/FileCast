...
function markdownToHtml(md) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (inCodeBlock) {
      if (line.trim() === '```') {
        html += '<pre><code' + (codeLang ? ' class="language-' + codeLang + '"' : '') + '>' +
                escapeHtml(codeLines.join(NL)) + '</code></pre>' + NL;
        inCodeBlock = false;
        codeLines = [];
        codeLang = '';
      } else {
    ...
    if (line.trim().indexOf('```') === 0) {
      ...
      if (inBlockquote) { html += processBlockquote(blockquoteLines); inBlockquote = false; blockquoteLines = []; }
      inCodeBlock = true;
    ...
    if (headingMatch) {
      ...
      html += '<h' + level + '>' + processInline(headingMatch[2].trim()) + '</h' + level + '>' + NL;
      continue;
    }

    if (line.trim().match(/^---+$/) || line.trim().match(/^\*\*\*+$/) || line.trim().match(/^___+$/)) {
      if (inList) { html += '</' + listType + '>' + NL; inList = false; }
      html += '<hr>' + NL;
      continue;
    }
...
function processBlockquote(lines) {
  var NL = String.fromCharCode(10);
  return '<blockquote>' + NL + '<p>' + lines.map(processInline).join('<br>') + '</p>' + NL + '</blockquote>' + NL;
}
