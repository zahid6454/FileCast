...
CONVERT_TOOLS = {
    "docx-to-pdf": ("test.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
    "xlsx-to-pdf": ("test.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
    "pptx-to-pdf": ("test.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation"),
...
class Report:
    ...
    def summarize(self, label: str, wall_time: float) -> None:
        ...
        client_err = [r for r in self.results if r.status and 400 <= r.status < 500 and r.status != 429]
        ...
        if latencies:
            p = lambda pct: latencies[min(len(latencies) - 1, int(len(latencies) * pct))]
            print(
...
async def _one_convert(client: httpx.AsyncClient, tool: str, filename: str, content: bytes, mime: str) -> Result:
    t0 = time.monotonic()
    try:
        ...
        return Result(status=None, elapsed=time.monotonic() - t0, error=f"{type(e).__name__}: {e}")
...
async def run_convert_all(base_url: str, concurrency: int, total: int) -> None:
    for tool in CONVERT_TOOLS:
        await run_convert(base_url, tool, concurrency, max(1, total // len(CONVERT_TOOLS)))
...
async def _one_post_json(client: httpx.AsyncClient, path: str, json_body: dict) -> Result:
    t0 = time.monotonic()
    try:
...
async def run_data(base_url: str, concurrency: int, total: int) -> None:
    ...
    async def worker(client: httpx.AsyncClient, i: int) -> None:
        async with sem:
            if i % 4 == 0:
                ...
                report.add(
                    await _one_post_json(
                        client, "/api/v1/ratings", {"tool_id": "stress-test-tool", "vote": "yes"}
                    )
                )
    ...
    report.summarize(f"data-layer mixed GET/POST  (concurrency={concurrency}, n={total})", wall)
...
async def run_ratelimit_probe(base_url: str, tool: str) -> None:
    ...
    20/hr = up to ~80/hr) is real in practice. A single kept-alive connection
    ...
    statuses: list[int] = []
    for i in range(100):
        async with httpx.AsyncClient(base_url=base_url, headers={"Connection": "close"}) as client:
    ...
    print(f"configured limit : 20/hr (single worker)")
...
def main() -> None:
    ...
    parser.add_argument("--base-url", default="http://localhost:8090", help="Never point this at production.")
    sub = parser.add_subparsers(dest="mode", required=True)

    p_conv = sub.add_parser("convert", help="Load-test server-side conversion endpoints (Gotenberg/Ghostscript).")
    ...
    p_data = sub.add_parser("data", help="Load-test data-layer endpoints (DB pool, rate limiter).")
    p_data.add_argument("--concurrency", type=int, default=50)
    p_data.add_argument("--requests", type=int, default=500)

    p_rl = sub.add_parser("ratelimit", help="Sequential probe of one endpoint's rate-limit boundary.")
    ...
    if "localhost" not in args.base_url and "127.0.0.1" not in args.base_url:
        print(f"Refusing to run against non-local base-url: {args.base_url}", file=sys.stderr)
    ...
    if args.mode == "convert":
        if args.tool == "all":
            ...
            asyncio.run(run_convert(args.base_url, args.tool, args.concurrency, args.requests))
    elif args.mode == "data":
